...
int main()
{
  ...
  open_viii::Paths::for_each_path([](const std::filesystem::path &path) {
    ...
    const auto battle = archives.get<open_viii::archive::ArchiveTypeT::battle>();
    battle.execute_on({ open_viii::battle::stage::X::EXT },
      []([[maybe_unused]] std::vector<char> &&rvalue_buffer, std::string &&rvalue_path) {
        open_viii::battle::stage::X(std::move(rvalue_buffer), std::move(rvalue_path));
      });
  });

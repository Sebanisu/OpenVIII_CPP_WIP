...
#include <string_view>
void
  fiflfs_create(const std::filesystem::path src,
                ...
                const std::string_view      archive_name)
{
  ...
  std::cout << "Source directory compressed data: \n\t" << source_compress_dat
            << '\n';
  if (std::filesystem::exists(source_compress_dat)) {
    std::cout << "\t\tExists\n";
  } else {
    std::cout << "\t\tMissing\n";
    exit(1);
  ...
  if (std::filesystem::exists(source_names_dat)) {
    std::cout << "\t\tExists\n";
  } else {
    std::cout << "\t\tMissing\n";
    exit(1);
  ...
  std::fstream fs_fi(fi_name,
                     std::ios::out | std::ios::binary | std::ios::trunc);
  std::fstream fs_fl(fl_name,
                     std::ios::out | std::ios::binary | std::ios::trunc);
  std::fstream fs_fs(fs_name,
                     std::ios::out | std::ios::binary | std::ios::trunc);
  std::fstream fs_source_names(source_names_dat,
                               std::ios::in | std::ios::binary);
  std::fstream fs_source_compress(source_compress_dat,
                                  std::ios::in | std::ios::binary);
  ...
  while (true) {
    ...
    open_viii::archive::FI fi = open_viii::archive::append_entry(
      fs_fs, src / current_name, current_compression);
    open_viii::archive::append_entry(fs_fi, fi);
    open_viii::archive::append_entry(fs_fl,

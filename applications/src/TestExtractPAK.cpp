...
bool
  validate_and_create_directory(const std::filesystem::path &path)
{
  std::error_code ec;
  if (!std::filesystem::exists(path, ec)) {
    ...
    std::cerr << "Path is not a directory: " << path << " - " << ec.message()
              << std::endl;
    return false;
  }
  return true;
  ...
  main()
{
  std::string choice;
  std::cout
    << "Enter 'scan' to scan known paths or enter the path to a .pak file: ";
  std::getline(std::cin, choice);

  std::string dest_path_input;
  std::cout
    << "Enter the destination path for extraction (leave blank for 'movies'): ";
  std::getline(std::cin, dest_path_input);

  std::filesystem::path dest_path
    = dest_path_input.empty() ? "movies" : dest_path_input;

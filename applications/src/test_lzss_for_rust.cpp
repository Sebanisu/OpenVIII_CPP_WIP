  ...
  test_compress_decompress()
{
  ...
  for (const auto &original_data : original_data_values) {
    ...
    std::vector<char> compressed_data
      = open_viii::compression::LZSS::compress(original_data);

    std::cout << "(\""
              << std::string_view(original_data.data(), original_data.size())
              << "\", \n";
    std::cout << "vec![";
    ...
    std::vector<char> decompressed_data
      = open_viii::compression::LZSS::decompress(
        compressed_data,
        original_data.size());

    // Assert that decompressed data matches original data
    if (
      std::string_view(decompressed_data.data(), decompressed_data.size())
      != original_data) {

...
std::optional<std::decay_t<T>>
  get_number(const std::string_view num, const int base = 10) noexcept
{
    ...
    const std::string_view &tile_type_view)
{
  ...
  const auto map = Map([&is, &tile_type_view]() -> typename Map::variant_tile {
    std::string line{};
    std::getline(is, line);
    if (std::empty(line))
    ...
    auto       variant_tile = get_tile(tile_type_view[0], raw_bytes);
    std::visit(
      [&](auto &tile) {

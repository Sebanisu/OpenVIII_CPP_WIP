...
public:
  explicit ComplexStringSectionEntry(const std::string_view & buffer)
  {
    ...
    std::memcpy(length_.data(),buffer.data()+sizeof(unknown_),sizeof(length_));
...
public:
  ...
  [[nodiscard]] auto number() const noexcept {return number_;}
  friend std::ostream & operator << (std::ostream &os, const ComplexStringSectionOffsets & item )
  {
    return os << '{' << item.offset() <<", "<< item.number() '}'<<
  }
};
...
public:
  explicit ComplexStringSection(const std::string_view & buffer)
  {
  ...
  [[nodiscard]] ComplexStringSectionEntry at(const ComplexStringSectionOffsets & offsets) const
  {
    return {std::string_view {buffer_.data()+offsets.offset(),buffer_.size()+offsets.offset()}}
  }

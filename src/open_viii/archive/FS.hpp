...
// */
// template<is_default_constructible_has_data_size_resize dstT =
// std::vector<char>,
//         FI_Like                                       fiT  = FI>
...
//    // size compressed isn't quite known with out finding the offset of the
//    next
...
//      // the size of the compressed data is the first value minus 8. the
//      second
...
//    return {};
//  }
//  // todo if we get a span stream we maybe can combine the two functions
//  into 1.
//  // stringstream only works with strings. and strstream is depreciated. It
//  // should use the other one. Though that is only the case for FIFLFS
//  archives. if (fi.uncompressed_size() == 0) {
//    return {};
//  }

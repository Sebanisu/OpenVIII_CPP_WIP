...
std::vector<std::filesystem::path> open_viii::Paths::get_windows_ff8_paths() {
    ...
    HKEY hKey;
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\Square Soft, Inc\\FINAL FANTASY VIII\\1.00", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        std::array<char, 512U> buffer = {};
        DWORD bufferSize = static_cast<DWORD>(buffer.size());
        if (RegQueryValueExA(hKey, "AppPath", nullptr, nullptr, reinterpret_cast<LPBYTE>(buffer.data()), &bufferSize) == ERROR_SUCCESS) {
    ...
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\Valve\\Steam", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {       
        std::array<char, 512U> buffer = {};
        DWORD bufferSize = static_cast<DWORD>(buffer.size());
        if (RegQueryValueExA(hKey, "SteamPath", nullptr, nullptr, reinterpret_cast<LPBYTE>(buffer.data()), &bufferSize) == ERROR_SUCCESS) {
            steam_path = std::string(buffer.data(), bufferSize - 1);  // Remove null terminator
        }
        RegCloseKey(hKey);
    }
 if (!steam_path.empty()) {
        std::vector<std::filesystem::path> libraries;
        std::filesystem::path main_steam_path = std::filesystem::path(steam_path) / "steamapps";
        ...
        if (std::filesystem::exists(vdf_path)) {
            ...
            while (std::getline(vdf_file, line)) {
                if (auto match = ctre::search<R"ccc("path"\s+"([^"]+)")ccc">(line)) {
                    std::filesystem::path lib_path = std::filesystem::path(match.get<1>().to_string()) / "steamapps";
        ...
        for (const auto& app_id : app_ids) {
            for (const auto& lib : libraries) {
                std::filesystem::path manifest_path = lib / ("appmanifest_" + app_id + ".acf");
                if (std::filesystem::exists(manifest_path)) {
                    std::ifstream manifest_file(manifest_path);
                    std::string line;
                    while (std::getline(manifest_file, line)) {
                        if (auto match = ctre::search<R"ccc("installdir"\s+"([^"]+)")ccc">(line)) {
                            std::filesystem::path path = lib / "common" / match.get<1>().to_string();                            
 ...
 std::vector<std::filesystem::path> &
  ...
  void open_viii::Paths::for_each_path(std::move_only_function<open_viii::Paths::Ops(std::filesystem::path)> lambda)
  {
